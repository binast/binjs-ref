...
impl TokenWriter for PathTraceWriter {
    ...
    fn identifier_name_at(&mut self, value: Option<&IdentifierName>, path: &IOPath) -> Result<(), TokenWriterError> {
        self.trace.borrow_mut().push(Step::IdentifierNameAt {
            value: value.cloned(),
            path: path.clone()
        });
        Ok(())
    ...
    fn string_at(&mut self, value: Option<&SharedString>, path: &IOPath) -> Result<(), TokenWriterError> {
        self.trace.borrow_mut().push(Step::StringAt {
            value: value.cloned(),
            path: path.clone()
        });
        Ok(())
    ...
    fn string_enum_at(&mut self, value: &SharedString, path: &IOPath) -> Result<(), TokenWriterError> {
        self.trace.borrow_mut().push(Step::StringEnumAt {
            value: value.clone(),
            path: path.clone()
        });
        Ok(())
    ...
    fn bool_at(&mut self, value: Option<bool>, path: &IOPath) -> Result<(), TokenWriterError> {
        self.trace.borrow_mut().push(Step::BoolAt {
            value: value.clone(),
            path: path.clone()
        });
        Ok(())
    ...
    fn unsigned_long_at(&mut self, value: u32, path: &IOPath) -> Result<(), TokenWriterError> {
        self.trace.borrow_mut().push(Step::UnsignedLongAt {
            value: value.clone(),
            path: path.clone()
        });
        Ok(())
    ...
    fn list_at(&mut self, items: Vec<()>, path: &IOPath) -> Result<Self::Tree, TokenWriterError> {
        self.trace.borrow_mut().push(Step::ListAt {
            len: items.len(),
            path: path.clone()
        });
        Ok(())
    ...
    fn float_at(&mut self, value: Option<f64>, path: &IOPath) -> Result<Self::Tree, TokenWriterError> {
        self.trace.borrow_mut().push(Step::FloatAt {
            value: value.clone(),
            path: path.clone()
        });
        Ok(())
    ...
    fn property_key_at(&mut self, value: Option<&PropertyKey>, path: &IOPath) -> Result<Self::Tree, TokenWriterError> {
        self.trace.borrow_mut().push(Step::PropertyKeyAt {
            value: value.cloned(),
            path: path.clone()
        });
        Ok(())
...
test!(test_es6_paths, {
    ...
    let mut properties : Vec<_> = borrow
        .iter()
        .filter_map(|step| {
            if let Step::PropertyKeyAt { ref value, ref path } = *step {
