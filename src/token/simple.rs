...
use ast::grammar::{ Field, InterfaceNode, Syntax };
use token::io::*;
...
impl TokenWriter for TreeTokenWriter {
    ...
    /// - contents
    fn tagged_tuple(&mut self, mut children: Vec<Self::Tree>, node: &InterfaceNode) -> Result<Self::Tree, Self::Error> {
        let mut prefix = Vec::new();
        prefix.extend_from_slice(&"<head>".as_bytes());
        let kind = node.kind()
            .ok_or_else(|| TokenWriterError::MissingKind)?;
        ...
        prefix.extend_from_slice(&"</head>".as_bytes());
        children.insert(0, Rc::new(prefix));

        self.untagged_tuple(children)
    }
    fn untagged_tuple(&mut self, children: Vec<Self::Tree>) -> Result<Self::Tree, Self::Error> {
        let mut result = Vec::new();
