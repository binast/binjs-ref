...
impl CPPExporter {
    pub fn new(deanonymizer: TypeDeanonymizer, files: FileNames, options: SpecOptions) -> Self {
...
// - cargo run -- $(topsrcdir)/js/src/frontend/{webidl} $(topsrcdir)/js/src/frontend/{yaml}
...
",
            class = self.files.class,
            impl_ = self.files.impl_,
            token = self.files.token,
            webidl = self.files.webidl,
            yaml = self.files.yaml,
        );
        warning
    }
...
fn main() {
    ...
    let files = FileNames {
        class: apparent_file_name("?", matches.value_of("OUT_HEADER_CLASS_FILE")),
        impl_: apparent_file_name("?", matches.value_of("OUT_IMPL_FILE")),
        token: apparent_file_name("?", matches.value_of("OUT_TOKEN_FILE")),
        webidl: apparent_file_name(source_path, matches.value_of("OUT_WEBIDL_FILE")),
        yaml: apparent_file_name(matches.value_of("INPUT.yaml").unwrap(), matches.value_of("OUT_YAML_FILE")),
    };
