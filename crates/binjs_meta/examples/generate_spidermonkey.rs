...
fn main() {
    ...
    let matches = App::new("BinJS import from webidl")
        ...
        .args(&[
            ...
            Arg::with_name("OUTPUT")
                .required(true)
                .help("Prefix of output files to use. OUTPUT-class.h, OUTPUT-token.h and OUTPUT.cpp will be produced."),
        ])
    .get_matches();
    ...
    println!("...exporting C++ class header code: {}-class.h", dest_path);
    ...
    println!("...exporting C++ token header code: {}-token.h", dest_path);
    let mut dest = File::create(format!("{}-token.h", dest_path))
        .expect("Could not create C++ header source output");
    dest.write_all(exporter.to_spidermonkey_token_hpp().as_bytes())
        .expect("Could not write C++ header source output");

    println!("...exporting C++ implementation code: {}.cpp", dest_path);
