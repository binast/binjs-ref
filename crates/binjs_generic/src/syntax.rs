    ...
    fn compare(&self, syntax: &Spec, left: &JSON, right: &JSON) -> Result<bool, ASTError> {
        ...
        match (self, left, right) {
            (&TypeSpec::Boolean, &Boolean(ref a), &Boolean(ref b)) =>
                Ok(a == b),
            (&TypeSpec::String, _, _) | (&TypeSpec::IdentifierReference, _, _) | (&TypeSpec::IdentifierDefinition, _, _) if left.as_str().is_some() && right.as_str().is_some() => // Strings are complicated as they have two different representations in JSON.
        ...
        impl<'a, V> $walker_name<'a, V> where V: $visitor_name {
            ...
            fn walk_type_spec_aux(&mut self, value: & $($mutability)* JSON, spec: &TypeSpec, name: &NodeName) -> Result<(), ASTError> {
                match spec {
                    ...
                    &TypeSpec::String | &TypeSpec::IdentifierReference | &TypeSpec::IdentifierDefinition  => {
